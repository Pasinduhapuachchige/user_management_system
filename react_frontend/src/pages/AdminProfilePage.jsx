...
const FamilyInfoCard = ({ adminData }) => {
    const maritalStatus = adminData?.maritalStatus || 'Unmarried';

    const StatusBadge = ({ value }) => {
        const colors = {
            Alive: 'bg-green-100 text-green-700 border border-green-200',
            Deceased: 'bg-red-100 text-red-600 border border-red-200',
        };
        return (
            <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${colors[value] || 'bg-gray-100 text-gray-600 border border-gray-200'}`}>
                {value}
            </span>
        );
    };

    const RelationBadge = ({ value }) => (
    ...
    const EmptyState = ({ label }) => (
        <div className="text-center py-5 px-4 border-2 border-dashed border-gray-100 rounded-xl text-gray-400 text-sm italic">
            No {label} recorded.
        </div>
    );
        ...
        <div className="border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-8">
                ...
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                    <div className="flex flex-wrap items-center gap-4">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Marital Status</span>
                            <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                                maritalStatus === 'Married' ? 'bg-pink-100 text-pink-700 border border-pink-200'
                                : maritalStatus === 'Divorced' ? 'bg-orange-100 text-orange-700 border border-orange-200'
                                : 'bg-blue-100 text-blue-700 border border-blue-200'
                            }`}>
                                {maritalStatus}
                            </span>
                        </div>
                        {adminData?.spouseName && (
                            <div className="flex items-center gap-2">
                                <User className="w-4 h-4 text-gray-400" />
                                <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Spouse</span>
                                <span className="text-sm font-semibold text-gray-800">{adminData.spouseName}</span>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    {adminData?.parents?.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {adminData.parents.map((parent, index) => (
                                <div key={parent._id || index} className="border border-gray-200 rounded-xl p-4 bg-white hover:border-blue-200 hover:shadow-sm transition-all">
                                    <p className="font-bold text-gray-900 mb-2">{parent.name || 'N/A'}</p>
                                    <div className="flex flex-wrap gap-1.5">
                                        {parent.relationship && <RelationBadge value={parent.relationship} />}
                                        {parent.status && <StatusBadge value={parent.status} />}
                                    </div>
                        ...
                        <EmptyState label="parents" />
                    )}
                </div>

                {/* ── Spouse's Parents ─────────────────────────────────── */}
                {adminData?.spouseParents?.length > 0 && (
                    <div>
                        <h3 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2 uppercase tracking-wider">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {adminData.spouseParents.map((parent, index) => (
                                <div key={parent._id || index} className="border border-gray-200 rounded-xl p-4 bg-white hover:border-violet-200 hover:shadow-sm transition-all">
                                    <p className="font-bold text-gray-900 mb-2">{parent.name || 'N/A'}</p>
                                    <div className="flex flex-wrap gap-1.5">
                                        {parent.relationship && (
                ...
                <div>
                    ...
                    {adminData?.children?.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {adminData.children.map((child, index) => (
                                <div key={child._id || index} className="border border-gray-200 rounded-xl p-4 bg-white hover:border-emerald-200 hover:shadow-sm transition-all">
                                    <p className="font-bold text-gray-900 mb-2">{child.name || 'N/A'}</p>
                                    <div className="flex flex-wrap gap-1.5 mb-2">
                                        ...
                                        {child.status && <StatusBadge value={child.status} />}
                                    </div>
                                    {child.dateOfBirth && (
